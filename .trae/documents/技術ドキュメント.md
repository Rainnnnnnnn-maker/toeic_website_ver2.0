...
ユーザー（ブラウザ）
  └─ 表示・操作
       └─ Next.js（App Router）
            ...
            │         ├─ L1: Next.js Data Cache（"use cache" + cacheLife("weeks")）
...
#### データ要件
- 単語一覧は `./.doc/word.txt`（重要単語）と `./.doc/word_mid.txt`（中重要単語）をソースとして読み込む
  - 実装は `src/data/words.ts` で `process.cwd()` 基準に `".doc/<filename>"` を参照する
  - `slug` は `term.toLowerCase()` で生成し、同一slugは重複排除される
...
- 単語詳細ページ `src/app/words/[word]/page.tsx`
  ...
  - 存在しない単語/生成失敗時は、ページ内にエラーメッセージを表示する（現状は404にしない）
