  ...
  return (
    <section className="flex flex-col gap-4">
      <div className="mb-0">
        {query ? (
          <div>
             <h2 className="text-xl font-bold text-slate-800 mb-2">検索結果: {filtered.length}件</h2>
          </div>
        ) : activeTab === 'important' ? (
          <div>
            <h2 className="text-xl font-bold text-slate-800 mb-2">最重要単語（TOEIC 600点レベル）</h2>
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800 mb-2">中級単語（TOEIC 730〜800点レベル）</h2>
            <p className="text-sm leading-relaxed text-slate-600">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800 mb-2">高難易度単語（TOEIC 800点以上レベル）</h2>
            <p className="text-sm leading-relaxed text-slate-600">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative w-full flex-1 sm:mr-2.5">
          <div className={`absolute bottom-full left-0 mb-2 bg-slate-400/30 text-slate-800 px-3 py-2 rounded-lg text-xs font-medium pointer-events-none opacity-0 translate-y-1 transition-all duration-200 whitespace-nowrap z-10 backdrop-blur-md shadow-sm border border-white/20 after:content-[''] after:absolute after:-bottom-1 after:left-6 after:w-2 after:h-2 after:bg-slate-400/30 after:rotate-45 after:backdrop-blur-md after:border-b after:border-r after:border-white/20 ${isFocused && !query ? 'opacity-100 translate-y-0' : ''}`}>
            💡 ワイルドカード（*）検索が利用可能です
          </div>
          <input
            ref={inputRef}
            ...
            className="w-full flex-1 border border-gray-300 rounded-xl px-3.5 py-3 pr-10 text-sm bg-white/90 transition-all duration-180 focus:outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-300/35"
            placeholder="全単語から検索..."
            aria-label="単語検索"
          />
          {query && (
            <button
              className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center bg-transparent border-none p-1 cursor-pointer text-gray-400 rounded-full transition-all duration-200 hover:bg-gray-100 hover:text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300"
              onClick={handleClear}
              aria-label="検索条件をクリア"
              type="button"
            >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {current.map((word) => (
          ...
          >
            ...
            <span className="mt-2 text-[11px] text-gray-400">クリックしてAIによる解説を見る</span>
          </Link>
        ))}
      </div>
    </section>
