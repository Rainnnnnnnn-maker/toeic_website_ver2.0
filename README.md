...
- **Study Modes**:
  ...
  - **Review Mode (Sign-In Required)**: Focus on words marked as favorites. Guests reaching `/review` — including by bookmark — get the shared `FavoritePracticeLoginGate` instead of a session; there is no guest fallback to the full favorites list.
- **Today’s 6 Picks**:
  ...
  - Word-detail navigation never calls the daily cache. Daily-picks links carry the displayed slug order as a JSON `picks` query. `resolveTodayNavigation()` validates the bounded, unique list against known words and preserves it through every previous/next link, even when the cached corpus version differs. Invalid or unavailable selections hide both navigation links and offer a return to `/today-words`; they never widen to the full word list. Valid legacy date/version links are upgraded to snapshots. No daily cache dependency is added to pre-rendered word pages.
- **Listen-and-Repeat Mode (Beta)** (`/today-words/listen` — open to everyone, `/favorites/listen` — sign-in required):
  ...
  - `FavoritesListenClient` is a thin state gate (guest → login gate, loading → skeleton, error → retry) and mounts `FavoritesListenPlayer` only once Supabase favorites are ready, so no TTS request is issued for a signed-out visitor.
...
- **Favorites**: Save difficult words for later review. Persisted in local storage via `FavoritesContext` for guests; synced to Supabase for signed-in users. Adding and browsing favorites never requires an account; only the favorites-powered review and listen modes do. On `/favorites`, signed-out visitors with at least one favorite see `FavoritePracticeLoginCard` (a single card linking to `/login?next=/mypage`) in place of the review/listen buttons.
  - `/favorites` has a prefix search box that filters the list by English term. Matching lives in the pure, unit-tested `src/lib/word-search.ts` (`filterWordsByTermPrefix`): input is NFKC-normalized, whitespace-collapsed, trimmed, and lowercased so full-width IME input still matches; exact matches come first and the newest-first order is otherwise kept. Substring and wildcard (`*`) matching are intentionally not supported here. The query never enters the URL or analytics. The bulk-delete button is labeled 「すべて削除」 so it is not mistaken for clearing the search, and it still removes every favorite behind the existing confirmation modal.
...
- **CDN-Cached OG Images (Active CPU control)**: Under `cacheComponents: true`, metadata image routes are never prerendered — even a route with no data access stays dynamic (`ƒ`), `export const revalidate` is inert, and `generateStaticParams` does not help; the build's `prerender-manifest.json` lists zero OG routes. The word OG route therefore carries **no** `generateStaticParams` / `dynamicParams = false`: keeping them only doubled the build's static-generation set (1,422 → 2,798 pages) with per-word prerender attempts that emit nothing, measurably degrading local builds (static generation 9.1s → 28.2s, whole build 17s → 35s, macOS compressed memory 4.7 GB → 11.4 GB, swapins 14k → 201k). Unknown slugs are rejected inside the Function by the `getWordBySlug` check, which returns a bare 404 with `OG_IMAGE_NOT_FOUND_CACHE_CONTROL` instead of rasterizing a "Not Found" image. Since each image costs a Satori layout pass plus a resvg 1200×630 PNG rasterization, leaving this unaddressed burns Vercel Fluid **Active CPU** in proportion to request volume. Both OG routes therefore send `OG_IMAGE_CACHE_CONTROL` (`src/lib/og-utils.ts`), letting the CDN hold the PNG so the Function runs roughly once per edge instead of once per request. `s-maxage` is 30 days to match the word-detail cache (`WORD_CACHE_TTL_DAYS`, `cacheLife("max")`), with `stale-while-revalidate` covering expiry. `loadGoogleFont` is wrapped in `'use cache'` and fetches one stable full-TTF URL shared by every OGP. It deliberately omits Google Fonts' `text=` parameter: that parameter produced a different cache key per word while returning the same roughly 7 MB font, duplicating identical data across 1,300+ entries and exhausting build memory.
...
- **Responsive Design**: Optimized for desktop and mobile devices. Below 640px, word-detail audio/favorite buttons and home/favorites pagination have at least 44×44px tap targets; icon sizes and desktop sizing stay compact. (Updated: 2026-09-23)
...
- Vercel Git Integration is enabled (`git.deploymentEnabled: true`). Pushes to the Vercel Production Branch automatically deploy to Production. The existing Preview push workflow and manual Production workflow also remain available, so Git integration and Actions can both deploy.
- The `nodejs-e2e` CI job runs the two Cypress flows on an isolated production build without service secrets. Failures upload build/server logs, screenshots, and videos for 7 days. Other CI jobs skip the Cypress binary download.
- CI and Vercel Git deployments run independently. Adding E2E does not automatically block Production promotion. Configure required PR checks / Vercel Deployment Checks separately if needed. Workflow `paths-ignore` rules do not disable Vercel Git deployments.
...
- 苦手単語一覧は `WEAK_WORDS_LIMIT`（現在7語）まで表示する。× は「覚えていない」の累計回数と明記し、開始時の上限を適用した対象語数をボタンに表示する（途中再開時は保存済みセッションが優先）。
...
ヘッダー直下に「今日のおすすめ単語」を置き、「はじめての方へ」、意味検索、単語一覧の順で表示します。初回学習の案内は文字・余白・ボタンをコンパクトにし、モバイルではボタンの折り返しと高さ44px以上のタップ領域を確保しています。
