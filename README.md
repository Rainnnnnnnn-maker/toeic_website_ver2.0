...
- **Adaptive Word-Detail Disclosures**: A word's primary Japanese translation always stays visible, while only dense supporting content is collapsed. Native `<details>` panels show a one-line preview, item count, and explicit 「見る／閉じる」state for long meaning breakdowns, examples, notes, and tag lists; short sections remain open without adding an unnecessary interaction. The pure length/count rules live in `src/lib/word-detail-disclosure.ts` and keep the behavior consistent across AI-generated payloads.
- **Smart Caching**:
