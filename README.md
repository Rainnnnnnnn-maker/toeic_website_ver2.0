...
- **My Page & Review Flow (Sign-In Required)** (`/mypage`): A personal hub whose only job is to answer “what should I review today, and why now?”. Account information is deliberately minimal (avatar, display name, sign-out); the screen is dominated by the review flow for favorite words. Signed-in users get a Leitner-style schedule (1 / 3 / 7 / 14 / 30-day intervals) stored in Supabase (`word_review_progress`, `learning_streaks`, both RLS-protected), so the same due queue and streak follow them across devices. The page shows today's due count with a progress ring, a 🔥 streak / best-streak / weekly-reviewed row, a four-segment retention breakdown (untouched / learning / familiar / mastered), the most-forgotten words, and links into the existing favorites, listen, study, and daily-picks screens. Grading in review mode (`覚えている` / `覚えていない` / `あとで`) writes progress optimistically and pushes the next due date to the start of a later JST day, so reviewing at a slightly different time each evening never slips by a day. **Favorites are read strictly from Supabase**: the page renders only when `favoritesStatus === "ready"`, so the localStorage snapshot that `FavoritesContext` briefly returns right after login is never displayed here. Guests hitting `/mypage` get a login gate that also links to everything that stays free, and `/review?queue=due|weak` degrades to the full favorites list with a notice rather than blocking. All scheduling rules are pure functions in `src/lib/review-schedule.ts` (unit-tested), with Supabase access isolated in `src/lib/review-progress-repo.ts` and `src/hooks/useReviewProgress.ts`. Auth stays client-side (the proxy matcher is unchanged) so the page shell remains statically prerendered.
