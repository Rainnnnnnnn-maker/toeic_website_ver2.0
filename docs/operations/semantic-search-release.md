...
- [ ] 「感謝を伝えるメールで使う単語」のような場面クエリでもそれらしい結果が出る
- [ ] 結果カードから `/words/[slug]` に遷移できる
- [ ] 同じクエリを2回検索し、2回目が速い（DevTools でレスポンスの `cached: true` を確認）
- [ ] 「英単語で探す」タブに戻して既存の前方一致・`*` 検索が壊れていない
- [ ] TOPのAI検索カード：入力して送信すると `/words` の意味検索タブへ遷移し、同じクエリが自動実行される（例クエリチップのタップでも同様）
- [ ] 遷移先URLが `/words?mode=meaning&launch=<不透明ID>` となり検索語本文を含まない。同じタブで再読み込み・戻る・進むを行っても検索語を復元できる
- [ ] URLだけを別タブへコピーした場合は検索語を復元・自動実行せず、`/words` の「意味で探す」タブだけを表示する
- [ ] TOPへ戻った後も送信ボタンと例クエリチップが操作でき、開発時も同一クエリの初回POSTが1回だけである
