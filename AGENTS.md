...
- UI: 「意味で探す」tab in `WordsExplorerClient` renders `SemanticSearchPanel` (client). Results link to `/words/[slug]`. The TOP page hosts `SemanticSearchLauncher` (no results UI) which navigates to `/words?mode=meaning&q=...#word-explorer`; `WordsExplorerClient` reads those params via `useSyncExternalStore` on `location.search` (SSR renders term mode; no setState-in-effect, static rendering preserved) and `SemanticSearchPanel` auto-runs `initialQuery` once on mount. URL param logic lives in `src/lib/semantic-launch.ts` — a client-safe pure module (no `crypto` import, unlike `semantic-search.ts`).
