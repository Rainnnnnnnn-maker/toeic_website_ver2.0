...
- A known-good Turbopack build (2026-09-01) compiled in about 3 seconds, type-checked in about 2 seconds, and generated about 1,420 static pages in about 9 seconds. The count is roughly the word list plus the non-word routes; it used to be about double that because `opengraph-image.tsx` carried a `generateStaticParams` whose prerender attempts produced nothing. If a build ever reports twice the expected page count again, look for a metadata image route that grew static params rather than assuming the corpus doubled. The page count alone is not a reason to wait through a silent multi-minute compile.
...
**Do not call `getTodayRecommendedSelection()` / `getTodayRecommendedWords()` from anything reachable by `/words/[word]`** — including its `layout.tsx`. Those pages are fully pre-rendered by `generateStaticParams` (~2,800 pages); depending on the daily `today-recommended-words` tag there would make every one of them a revalidation candidate on each Cron fire, which is what blew up Vercel ISR Writes. Instead, links out of the daily-picks UI carry the displayed slug order as a JSON `picks` query (`?from=today&picks=...`). `buildTodayNavigationQuery()` and `resolveTodayNavigation()` in `src/lib/today-navigation.ts` preserve that order against the long-lived `getAllWords()` list already passed as props. Do not reselect the words from a client date or require a matching corpus fingerprint for these snapshot links.

Validate that snapshots are nonempty, contain at most `TODAY_WORDS_COUNT` unique known slugs, and include `currentSlug`. If any validation fails, hide previous/next links and offer `/today-words`; never fall back to all words or reuse previously saved navigation state in the today context. Existing date/version links can still be restored by `resolveTodayNavigationSelection()` when their date, corpus fingerprint and membership match; successful legacy navigation is upgraded to a snapshot query. Missing or mismatched legacy context also shows the recovery link. The daily selection function retains its date/version metadata for compatibility, but new UI links use the displayed words directly.
...
`/mypage` is the review hub for favorite words, and it is the one feature that **requires sign-in**. Everything else stays usable for guests — do not gate existing features on auth.

- **Favorites shown here come from Supabase only.** `FavoritesContext` exposes `favoritesStatus` (`guest` / `loading` / `ready` / `error`); render sign-in-only screens only when it is `ready`. Right after login `favorites` still returns the localStorage snapshot until the remote fetch resolves, so skipping the gate would briefly display the wrong list.
- **Scheduling logic is pure and lives in `src/lib/review-schedule.ts`** (Leitner boxes 1..5 = 1/3/7/14/30 days, due ordering, summaries, streaks, Supabase row conversion). It has no `server-only` marker and no `"use client"`. Keep new rules there with unit tests; never duplicate the JST day boundary — reuse `getTodayKey()` from `word-select.ts`. Next-due timestamps are snapped to the start of a later JST day so a nightly learner does not drift by a day.
- **Supabase access is isolated in `src/lib/review-progress-repo.ts` and `src/hooks/useReviewProgress.ts`** (tables `word_review_progress` / `learning_streaks`, RLS identical to `favorites`). Reads and writes happen in the browser with `src/lib/supabase/client.ts`. Grading is optimistic, but not fire-and-forget: write the latest record/streak to the user-scoped `review-progress-outbox.ts` localStorage outbox before sending, serialize flushes per user so rapid grades cannot finish out of order, acknowledge only the exact value that succeeded, and merge/retry pending values on the next hook mount or grade. The streak row is queued only on the first grade of the day. `learning_streaks.last_study_date` is a JST date computed client-side — never `now()::date`, which is UTC and would break streaks for study sessions between JST 0:00 and 9:00.
- **Auth is still resolved client-side.** Do not add `/mypage` to the `src/proxy.ts` matcher and do not read the user in the Server Component: the page shell must stay statically prerendered, and without proxy-based refresh a server-side `getUser()` would report a signed-in user as a guest once the access token expires.
...
- **Every step that touches an upstream needs that mapping, not just the headline API call.** In a Route Handler it is easy to guard only the obvious call and let an earlier step throw straight out of the handler, which Next.js turns into a generic 500. In `/api/tts` the allowlist check walks the full L1 → L2 → Gemini chain via `getWordDetail`, so an uncached word surfaces Gemini 429s and timeouts there — before `callTtsApi` is ever reached. The Upstash-backed rate limiter can throw in both `/api/tts` and `/api/search/semantic`, and in the latter it runs *before* the main `try`. All three are wrapped. Rate-limiter failures deliberately fail closed (metered APIs must not be exposed while the limiter is blind); only the status is made accurate.
...
- For review-flow changes, additionally verify the guest login gate on `/mypage`, the `?queue=due` guest fallback, grading (including the "覚えていない" path that navigates away) writing to `word_review_progress`, streak counting once per day, and the same due queue appearing on a second device signed in to the same account.
