...
- **Supabase access is isolated in `src/lib/review-progress-repo.ts` and `src/hooks/useReviewProgress.ts`** (tables `word_review_progress` / `learning_streaks`, RLS identical to `favorites`). Reads and writes happen in the browser with `src/lib/supabase/client.ts`. Grading is optimistic + fire-and-forget; the streak row is written only on the first grade of the day. `learning_streaks.last_study_date` is a JST date computed client-side — never `now()::date`, which is UTC and would break streaks for study sessions between JST 0:00 and 9:00.
...
- `/review` accepts `?queue=due|weak|all`. `all` (and no parameter) must keep behaving exactly as before. `due`/`weak` apply only when signed in with progress loaded; guests, fetch errors, and empty results fall back to the full favorites list with an inline notice instead of blocking. Each queue owns its own `sessionStorage` key so progress from different queues never mixes.
- `StudyClient` takes optional `onGrade` / `wordDetailFrom`. `/study` passes neither, so learning mode is unchanged. `onGrade` must be called synchronously in the "覚えていない" handler, before the `router.push` unmounts the component.
