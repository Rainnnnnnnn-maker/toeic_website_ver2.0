...
`getTodayRecommendedWords(limit)` in `src/data/words.ts` is a Cache Component (`'use cache'` + `cacheTag('today-recommended-words')` + `cacheLife('max')`). It picks words deterministically by UTC-based JST date key + FNV-1a hash of each slug. The pick count is the shared constant `TODAY_WORDS_COUNT` in `src/lib/word-select.ts` (currently 6) — never hardcode it. **The daily rotation is triggered by Vercel Cron** at UTC 22:05 (= JST 7:05) via `/api/revalidate/today-words`, which calls `revalidateTag('today-recommended-words')`. `cacheLife` is deliberately `'max'` rather than `'days'`: time-based expiry would go stale on a different boundary than the Cron, so the Cron is the single update trigger. Client components that display these words are dumb — they receive the words as props, never run selection logic themselves.

**Do not call `getTodayRecommendedWords()` from anything reachable by `/words/[word]`.** Those pages are fully pre-rendered by `generateStaticParams` (~2,800 pages); depending on the daily `today-recommended-words` tag there would make every one of them a revalidation candidate on each Cron fire, which is what blew up Vercel ISR Writes. Instead, links out of the daily-picks UI carry the selected slugs in a `today` query parameter (`?from=today&today=<slug1,...>`), and `WordNavigationClient` validates them against the long-lived `getAllWords()` list to build prev/next. That is why the query string is long — it keeps daily-changing data out of the static render tree. A missing or fully invalid `today` query (old bookmark, day rollover) falls back to full-word-list navigation.
